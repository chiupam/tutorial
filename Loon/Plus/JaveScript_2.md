...
![#f03c15](https://placehold.it/15/f03c15/000000?text=+) 以此让用户学会如何手动在 Loon 内添加任何用户想添加的脚本
...
![#f03c15](https://placehold.it/15/f03c15/000000?text=+) 相关代码为图中 `[Script]` 下的代码，直接将代码复制粘贴到 Loon 配置文件的 `[Script]` 下即可

![image](https://raw.githubusercontent.com/chiupam/tutorial-image/master/Loon/Plus/JaveScript_2_3.jpg)

![#f03c15](https://placehold.it/15/f03c15/000000?text=+) 其中 `[MITM]` 下的 `hostname = ` ，意思为用户需要自行将 `hostname = ` 的 `=` 后面的地址（实际上就是域名）写入 Loon 配置文件内 `[MITM]` 的 `hostname = ` 后面，两个域名之间用英文逗号 `,` 分隔

![image](https://raw.githubusercontent.com/chiupam/tutorial-image/master/Loon/Plus/JaveScript_2_4.jpg)

![#f03c15](https://placehold.it/15/f03c15/000000?text=+) 复制/拷贝 `hostname = ` 的 `=` 后面的地址，然后粘贴到 Loon 配置文件内 `[MITM]` 的 `hostname = ` 后面，两个域名之间用英文逗号 `,` 分隔
